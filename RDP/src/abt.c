...
float TIMEOUT = 0.0;
int nextSeqNo = 0;
struct pkt packetFromA; // Packet to be sent from A
int pendingAck = 0;
int rcvNextSeqNo = 0;
  ...
  struct msg message;
{
	...
	if(pendingAck == 0){
		...
		strcpy(packetFromA.payload, message.data);
		...
		//Starting the timer after transmitting a packet
		starttimer(0, TIMEOUT);

		//Sending the message to layer 3
		printf("A:Sending packet with sequence number %d to layer 3\n",nextSeqNo);
  ...
  struct pkt packet;
{
	...
	if(packet.checksum == checkSum && packet.seqnum == nextSeqNo){
		printf("A:ACK received for Seq Number %d. Stopping timer\n",packet.seqnum);
		//Stopping the timer after an acknowledgement is received
		stoptimer(0);

		//Setting the sequence number of the next packet to be sent from A
		if(nextSeqNo == 0)
			nextSeqNo = 1;
		else
			nextSeqNo = 0;

		//Setting pending acknowlegement flag to 0
		pendingAck = 0;
	}
	// Ignore when a duplicate ACK is received
	else if(packet.checksum == checkSum){
		printf("A:Duplicate ACK received for Seq Number %d\n",packet.seqnum);
	}
	else{
		printf("A:Acknowledgement packet is corrupted\n");
	}
}
...
void A_timerinterrupt()
{
	...
	//Sending the latest packet again to layer 3
	printf("A:Timer Interrupt. Retransmitting packet with sequence number:%d\n",packetFromA.seqnum);
	tolayer3(0, packetFromA);
}  
  ...
  struct pkt packet;
{
	...
	checkSum = checkSum + packet.acknum;
	for(y = 0; y < 20; y++){
		checkSum += packet.payload[y];
	}

	//If checksum is correct
	if(packet.checksum == checkSum && packet.seqnum == rcvNextSeqNo){
		//Setting the sequence number of the next packet to be received from A
		if(rcvNextSeqNo == 0)
			rcvNextSeqNo = 1;
		else
			rcvNextSeqNo = 0;

		printf("B:Received Packet with Sequence number:%d\n",packet.seqnum);
		...
		printf("B:Sending ACK for Sequence number:%d\n",packet.seqnum);
		tolayer3(1,	packet); // simply sending the same packet

		// deliver the data to upper layer
		printf("B:Sending data for Sequence number to upper layer:%d\n",packet.seqnum);
		tolayer5(1, packet.payload);
	}else if(packet.checksum == checkSum){
		// Sending acknowledgement to A and message to layer 5
		printf("B:Sending Duplicate ACK for Sequence number:%d\n",packet.seqnum);
		tolayer3(1,	packet); // simply sending the same packet
	}else{
		printf("B:Corrupted Packet recieved\n");
	}
}
